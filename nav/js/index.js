...
$('.list').onclick = function (e) {
    let $delete = e.path.find($node => $node.classList && $node.classList.contains('icon-del'))
    if ($delete) {
...
function render(data) {
    let $itemArr = data.map(obj => {
        ...
        $iconSpan.innerHTML = `
        <svg class="icon icon-edit" aria-hidden="true"><use xlink:href="#icon-edit"></use></svg>
        <svg class="icon icon-del" aria-hidden="true"><use xlink:href="#icon-del"></use></svg>
        `
        $h2.append($iconSpan)
        const $ul = document.createElement('ul')
        $ul.classList.add('classify')
        let $liArray = obj.data.map(item => {
            const $tag = document.createElement('li')
            $tag.classList.add('tag')
            $tag.innerHTML = '<svg class="icon icon-remove" aria-hidden="true"><use xlink:href="#icon-del"></use></svg>'
